...
const styles: { default: string; mobile: string; desktopTrue: string; desktopFalse: string } = {
  ...
  desktopFalse: "lg:text-[#b6bcc8ff] lg:font-normal lg:text-xs lg:relative before:['*'] lg:before:absolute lg:before:w-full lg:transition-all lg:duration-500 lg:ease-linear lg:hover:before:border-2 lg:before:border-[#ff7d1aff] lg:before:top-[3.35rem]",
};
...
export default function NavComponent({ menus }: { menus: NavProps<string>[] }): ReactElement {
  ...
  return (
    <>
      ...
      <aside className={`${showNavigation ? "block" : "hidden"} fixed top-0 left-0 [background-image:linear-gradient(to_right,_white_65%,_rgba(0,0,0,0.75)_65%)] w-full h-full z-25 p-5 lg:block lg:relative lg:p-0 lg:bg-transparent lg:bg-none lg:w-fit lg:h-fit lg:z-0`}>
